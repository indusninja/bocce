...
{
    public class ToggleDebugModeInputCheckForKeyboard : IInputCheck, IEngineObject
    {
        ...
        public ToggleDebugModeInputCheckForKeyboard()
        {
			IsDebugMode = false;
			IsDrawable = false;
			DrawOrder = 0;
        ...
        public float DidInputHappen<T>(T previous, T current)
        {
            if (typeof(T) == typeof(KeyboardState))
            {
                ...
                var previousState = keyDownMethodInfo.Invoke(previous, new object[] { Keys.F12 });

                var keyUpMethodInfo = keyboardStateType.GetMethod("IsKeyUp");
                var currentState = keyUpMethodInfo.Invoke(current, new object[] { Keys.F12 });
